...
const Events: NextPage = () => {
  ...
  // Your contracts were deployed around block 27765510 (MyNFT) and 27765505 (MyToken)
  // Lisk RPC has 100,000 block limit, so we start from a reasonable block
  const deploymentFromBlock = 27765000n;
                ...
                <tbody>
                  {currentEvents.slice(0, 20).map((event, index) => (
                    <tr key={`${event.log.transactionHash}-${index}`}>
                      <td>
                        <Address address={event.args[0]} size="sm" />
                      </td>
                      <td>
                        <Address address={event.args[1]} size="sm" />
                      </td>
                      <td>
                        {eventType === "token" ? (
